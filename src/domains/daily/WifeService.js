...
// WifeService: Tự động vấn an Tri Kỷ (Hậu Cung) & nhận thưởng thân mật
class WifeService extends BaseService {
  ...
  async autoCallAllWives(isLoop = false) {
    if (!isLoop) {
      console.log('\n[TỰ ĐỘNG VẤN AN TRI KỶ & HẬU CUNG]');
    ...
    if (!isLoop) {
      console.log(`\n[Hoàn tất] Đã thực hiện ${callsMade} lượt vấn an Tri Kỷ.\n`);
    }

    return callsMade;
  }

  // Nhận thưởng Vàng mốc cấp thân mật

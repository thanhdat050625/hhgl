...
// EventRankPointService: Tự động kiểm tra & nhận quà sự kiện tích điểm hạn giờ
class EventRankPointService extends BaseService {
  ...
  async autoClaimAllEventRewards(isLoop = false) {
    if (!isLoop) {
      console.log('\n[TỰ ĐỘNG NHẬN QUÀ SỰ KIỆN TÍCH ĐIỂM HẠN GIỜ]');
    ...
    if (!isLoop) {
      console.log(`\n[Hoàn tất] Quét xong sự kiện tích điểm (${totalClaimed} phần thưởng đã nhận).\n`);
    }

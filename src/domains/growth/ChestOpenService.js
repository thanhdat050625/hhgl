...
// ChestOpenService: Tự động mở rương và hộp quà nhận Vàng & Cung Vận
class ChestOpenService extends BaseService {
  ...
  async openGoldAndCungVanChests(isLoop = false) {
    if (!isLoop) {
      console.log('\n[TỰ ĐỘNG MỞ RƯƠNG & HỘP QUÀ VÀNG / CUNG VẬN]');
    }
    ...
    if (!isLoop) {
      console.log(`\n[Hoàn tất] Đã mở thành công ${openedCount} rương/hộp quà.\n`);
    }

...
// NoteManager: Quản lý đọc/ghi và đồng bộ danh sách tài khoản qua Web Note

class NoteManager {
  constructor(options = {}) {
    this.noteUrl = options.noteUrl || process.env.NOTE_URL;
    if (!this.noteUrl) throw new Error('[NoteManager] NOTE_URL bắt buộc phải được cấu hình!');

    this.loginUrl = options.loginUrl || process.env.NOTE_LOGIN_URL;
    if (!this.loginUrl) throw new Error('[NoteManager] NOTE_LOGIN_URL bắt buộc phải được cấu hình!');

    this.saveUrl = options.saveUrl || process.env.NOTE_SAVE_URL;
    if (!this.saveUrl) throw new Error('[NoteManager] NOTE_SAVE_URL bắt buộc phải được cấu hình!');

    this.notePassword = options.notePassword || process.env.NOTE_PASSWORD;
    if (!this.notePassword) throw new Error('[NoteManager] NOTE_PASSWORD bắt buộc phải được cấu hình!');

    this.noteId = options.noteId || process.env.NOTE_ID;
    if (!this.noteId) {
      const parts = this.noteUrl.split('/').filter(Boolean);
      this.noteId = parts[parts.length - 1];
    }
    this.noteTitle = options.noteTitle || process.env.NOTE_TITLE || this.noteId;

    this.sessionCookie = null;
    this.sessionCookieExpiresAt = 0;
  ...
  // Đảm bảo session cookie hợp lệ trước khi ghi
  ...
  // Đăng nhập Web Note lấy session cookie
  ...
  // Tải nội dung từ URL Note công khai
  ...
  // Parse text note thành danh sách tài khoản
  parseAccounts(rawText) {
    ...
    // Regex fallback [email, pass, on/off, optional_server]
  ...
  // Lấy danh sách tài khoản từ Web Note
  async getAccounts() {
  ...
  // Lưu danh sách accounts vào Note

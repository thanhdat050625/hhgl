...
formAddAcc.addEventListener('submit', async (e) => {
  ...
  const serverIdVal = document.getElementById('input-add-server').value.trim();
  const serverId = parseInt(serverIdVal, 10);

  if (!email || !password || isNaN(serverId)) {
    alert('Vui lòng nhập đầy đủ cả 3 thông tin bắt buộc: Email, Mật khẩu và Server ID!');
    return;
  }
